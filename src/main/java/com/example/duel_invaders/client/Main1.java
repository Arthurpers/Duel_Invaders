...
import com.example.duel_invaders.launchPattern.ProtocoleActiveKeys;
...
public class Main1 extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        ProtocoleActiveKeys protocol = new ProtocoleActiveKeys();
        ServeurTCP servAssocie = new ServeurTCP(new UnContexte() , protocol , 6666 );
        servAssocie.start();
        while (servAssocie.getNbConnexions() == 0) {
            try {
                Thread.sleep(500); // attendre 1 seconde avant de réessayer
                System.out.println("Attente de connexion");
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }
        ClientTCP myClt = new ClientTCP("localhost", 6667 );

        if ( myClt.connecterAuServeur() && servAssocie.getNbConnexions()>0) {
            ...
            AnimationTimer timer = new AnimationTimer() {
                ...
                public void handle(long now) {
                    if (gameEngine.getGameState().equals("ON")) {
                        ...
                        gameEngine.update(protocol.getActiveKeys(), gc, player2, player1, now);
                        myClt.transmettreChaine(inputHandler.getActiveKeys().toString());
